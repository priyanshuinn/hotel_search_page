...
function submit_form(event){
    ...
    if((date_flexible.checked) && !(form.get("month"))){
...
function check_in_date() {
    let form = new FormData(document.getElementById("search_form"));
    const selected_date = new Date(form.get("checkin"));
    const today = new Date();
    const is_date_valid =
        (selected_date.getFullYear() >= today.getFullYear() &&
        selected_date.getMonth() >= today.getMonth() && 
        selected_date.getDay() >= today.getDay() );
    if (!is_date_valid) {
...
function check_out_date() {
    ...
    const is_date_valid =
        ...
        selected_date.getDay() > checkin_date.getDay() );
    if (!is_date_valid) {
